...
if "cta_variant" not in st.session_state:
    import random
    st.session_state.cta_variant = random.choice(["A", "B"])
...
.main-owner-icon {{
    background-image: url("data:image/svg+xml,%3Csvg width='64' height='64' viewBox='0 0 64 64' fill='none' xmlns='http://www.w3.org/2000/svg'%3E%3Ccircle cx='32' cy='32' r='32' fill='%236C5CE7' fill-opacity='0.1'/%3E%3Cpath d='M32 12C24.268 12 18 18.268 18 26C18 33.732 24.268 40 32 40C39.732 40 46 33.732 46 26C46 18.268 39.732 12 32 12ZM32 16C37.523 16 42 20.477 42 26C42 31.523 37.523 36 32 36C26.477 36 22 31.523 22 26C22 20.477 26.477 16 32 16ZM32 42C23.163 42 16 49.163 16 58H48C48 49.163 40.837 42 32 42Z' fill='%236C5CE7'/%3E%3Cpath d='M32 32L36 36L44 28' stroke='white' stroke-width='3' stroke-linecap='round' stroke-linejoin='round'/%3E%3C/svg%3E");
...
if "event" in st.query_params:
    ev_type = st.query_params["event"]
    if ev_type == "main_owner_promo_click":
        log_analytics("main_owner_promo_click", {
            "card_size": st.query_params.get("card_size", "unknown"),
            "variant": st.session_state.variant,
            "cta_variant": st.session_state.cta_variant,
            "location": st.query_params.get("location", "unknown")
        })
    # Clear the event from query params to avoid duplicate logs on refresh
    new_params = {k: v for k, v in st.query_params.items() if k != "event"}
...
with st.sidebar:
    ...
    # Mobile Tile A/B Variants
    mobile_headlines = {"A": "Main Owner Setup", "B": "Control Center"}
    mobile_ctas = {"A": "Start", "B": "Unlock"}

    m_h = mobile_headlines[st.session_state.variant]
    m_c = mobile_ctas[st.session_state.cta_variant]

    log_analytics("main_owner_promo_view", {
        "card_size": "mobile_tile",
        "variant": st.session_state.variant,
        "cta_variant": st.session_state.cta_variant,
        "location": "sidebar"
    })
    ...
    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=mobile_tile&location=sidebar" class="promo-card mobile-tile" aria-label="Main Owner Onboarding">
        <div class="card-icon main-owner-icon"></div>
        <div class="card-headline">{html.escape(m_h)}</div>
        <div class="card-cta">{html.escape(m_c)}</div>
    </a>
    """, unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    # Hero Card A/B Variants
    hero_headlines = {"A": "Master Your AI Fleet", "B": "Unified Control Starts Here"}
    hero_ctas = {"A": "Set Main Owner", "B": "Start Onboarding"}

    h_h = hero_headlines[st.session_state.variant]
    h_c = hero_ctas[st.session_state.cta_variant]
    ...
    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=hero&location=dashboard_top" class="promo-card hero-card" aria-label="Main Owner Onboarding">
        <div class="card-headline">{html.escape(h_h)}</div>
        ...
        <div class="card-cta">{html.escape(h_c)}</div>
    </a>
    """, unsafe_allow_html=True)
    ...
    # Compact Card A/B Variants
    compact_headlines = {"A": "Secure Your Workspace", "B": "Enable Owner Oversight"}
    compact_ctas = {"A": "Setup Now", "B": "View Guide"}

    c_h = compact_headlines[st.session_state.variant]
    c_c = compact_ctas[st.session_state.cta_variant]

    log_analytics("main_owner_promo_view", {
        "card_size": "compact",
        "variant": st.session_state.variant,
        "cta_variant": st.session_state.cta_variant,
        "location": "dashboard_mid"
    })

    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=compact&location=dashboard_mid" class="promo-card compact-card" aria-label="Main Owner Onboarding">
        <div class="card-icon main-owner-icon"></div>
        <div class="card-content">
            <div class="card-headline">{html.escape(c_h)}</div>
            <div class="card-body">Gain a single source of truth for agent sessions and governance.</div>
        </div>
        <div class="card-cta">{html.escape(c_c)}</div>
    </a>
    """, unsafe_allow_html=True)
    ...
    with col_right:
        ...
        # Compact Card Variant (Sidebar fallback/Alt)
        log_analytics("main_owner_promo_view", {
            "card_size": "compact",
            "variant": st.session_state.variant,
            "cta_variant": st.session_state.cta_variant,
        ...
        st.markdown(f"""
        <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=compact&location=dashboard_sidebar" class="promo-card compact-card" aria-label="Main Owner Onboarding" style="padding: 1rem; margin-top: 1rem;">
            <div class="card-content">
                <div style="font-weight: 700; color: var(--text-primary);">{html.escape(c_h)}</div>
                <div style="font-size: 0.8rem; color: var(--text-secondary);">Unify your agent's mission today.</div>
            </div>
            <div class="card-cta" style="padding: 0.4rem 1rem; font-size: 0.75rem;">{html.escape(c_c)}</div>
        </a>
        """, unsafe_allow_html=True)
...
# ─── MAIN OWNER PAGE ─────────────────────────────────────
    ...
    log_analytics("main_owner_setup_start", {"source": "navigation_click"})
    st.markdown('<div class="hero-header">Main Owner Onboarding</div>', unsafe_allow_html=True)
    ...
    with col_faq:
        ...
        for q, a in faqs:
            with st.expander(q):
                st.markdown(f'<section aria-label="{html.escape(q)}">{html.escape(a)}</section>', unsafe_allow_html=True)

    with col_spec:
        st.markdown("#### What to Expect Checklist")
        checklist = [
        ...
        for i, item in enumerate(checklist):
            if st.checkbox(item, key=f"expect_{i}"):
                completed_count += 1

        if completed_count == len(checklist):
            st.success("You're ready to set up the Main Owner!")
            if st.button("Finalize Setup Confirmation", use_container_width=True):
                log_analytics("main_owner_setup_complete", {"status": "success", "checklist_completed": True})
                st.balloons()
                st.toast("Main Owner configuration initialized.")
...
// Sample Payload
{
  "event": "main_owner_setup_complete",
  "properties": {
    "status": "success",
