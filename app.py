...
with st.sidebar:
    ...
    if "page" in query_params:
        target_page = query_params["page"].replace("+", " ").replace("Main Owner Guide", "Main Owner")
    ...
    # Establish A/B Variants for Main Owner Promotions
    ...
    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=mobile" target="_self" class="promo-card mobile-tile" style="margin: 0.5rem; text-decoration: none;">
...
def log_analytics(event_name, payload=None):
    ...
    # Handle incoming event from query parameters
    query_params = st.query_params
    if "event" in query_params:
        qp_event = query_params["event"]
        qp_page = query_params.get("page")
        qp_payload = {k: v for k, v in query_params.items() if k != "event" and k != "page"}
        # Clear query params to prevent re-logging on refresh
        st.query_params.clear()
        if qp_page:
            st.query_params["page"] = qp_page
        # Log the event from query params
        log_analytics(qp_event, qp_payload)
...
if page == "Dashboard":
    ...
    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=hero" target="_self" class="promo-card hero-card" style="text-decoration: none;">
    ...
    with col_right:
        ...
        st.markdown(f"""
        <a href="?page=Main+Owner&event=main_owner_promo_click&card_size=compact" target="_self" class="promo-card compact-card" style="text-decoration: none;">
            <div class="card-icon">{MAIN_OWNER_ICON}</div>
...
# ─── MAIN OWNER PAGE ─────────────────────────────────────
    ...
    log_analytics("main_owner_view", {
        "variant": st.session_state.get("variant", "A"),
        "cta_variant": st.session_state.get("cta_variant", "1")
    })
    ...
    <div class="api-section">
        <h4 style="color: var(--primary-light);">Value Proposition</h4>
