...
# Handle Click Tracking and Navigation via Query Parameters
    query_params = st.query_params

    if "clicked_promo" in query_params:
        p_size = query_params["clicked_promo"]
        p_variant = query_params.get("v", "Unknown")
        log_analytics("main_owner_promo_click", {"card_size": p_size, "variant": p_variant})
        # Clean up URL but preserve destination page if present
    ...
    # Persistent A/B Variants from session state
    v_headline = st.session_state.ab_variants["headline"]
    v_cta = st.session_state.ab_variants["cta"]

    mobile_headlines = {"A": "Main Owner Setup", "B": "Control Center"}
    mobile_ctas = {"1": "Start", "2": "Unlock"}

    st.markdown(f"""
    <a href="?page=Main+Owner+Guide&clicked_promo=mobile&v={v_headline}{v_cta}" target="_self" class="promo-card mobile-tile" style="margin: 0.5rem;">
        <div class="card-icon">{MAIN_OWNER_ICON}</div>
        <div class="card-headline">{mobile_headlines[v_headline]}</div>
        <div class="card-cta">{mobile_ctas[v_cta]}</div>
    </a>
    """, unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    st.markdown(f"""
    <a href="?page=Main+Owner+Guide&clicked_promo=hero&v={v_headline}{v_cta}" target="_self" class="promo-card hero-card">
        <div class="promo-tag">New Feature</div>
        <div class="card-headline">{hero_headlines[v_headline]}</div>
        ...
        <div class="card-cta">{hero_ctas[v_cta]}</div>
    </a>
    """, unsafe_allow_html=True)
    ...
    with col_right:
        ...
        st.markdown(f"""
        <a href="?page=Main+Owner+Guide&clicked_promo=compact&v={v_headline}{v_cta}" target="_self" class="promo-card compact-card">
            <div class="card-icon">{MAIN_OWNER_ICON}</div>
            <div class="card-content">
                <div class="card-headline">{compact_headlines[v_headline]}</div>
                <div class="card-body">Unify your agent's mission today.</div>
            </div>
            <div class="card-cta" style="padding: 0.4rem 1rem; font-size: 0.75rem;">{compact_ctas[v_cta]}</div>
        </a>
        """, unsafe_allow_html=True)
...
# ─── MAIN OWNER PAGE ─────────────────────────────────────
    ...
    with col_s3:
        ...
        """, unsafe_allow_html=True)
        if st.button("Finalize Setup", use_container_width=True, key="finalize_setup_btn"):
            log_analytics("main_owner_setup_complete")
            st.success("Main Owner setup finalized successfully!")
