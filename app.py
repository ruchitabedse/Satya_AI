...
# Stable A/B Test Variant Selection
...
with st.sidebar:
    ...
    # Mobile Tile Promo (using stable A/B variants)
    ...
    # Check for clicks via query params
    if st.query_params.get("event") == "main_owner_promo_click" and st.query_params.get("card") == "mobile":
        log_analytics("main_owner_promo_click", {"card_size": "mobile", "variant": variant})

    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card=mobile" class="promo-card mobile-tile" aria-label="Navigate to Main Owner setup guide" target="_self">
...
if page == "Dashboard":
    ...
    # Main Owner Promo Hero Card (using stable A/B variants)
    variant = st.session_state.variant
    cta_variant = st.session_state.cta_variant
    ...
    log_analytics("main_owner_promo_view", {"card_size": "hero", "variant": variant, "headline": hero_headlines[variant]})

    # Check for clicks via query params
    if st.query_params.get("event") == "main_owner_promo_click" and st.query_params.get("card") == "hero":
        log_analytics("main_owner_promo_click", {"card_size": "hero", "variant": variant})

    st.markdown(f"""
    <a href="?page=Main+Owner&event=main_owner_promo_click&card=hero" class="promo-card hero-card" aria-label="{hero_headlines[variant]} - {hero_ctas[cta_variant]}" target="_self">
    ...
    with col_right:
        ...
        # Check for clicks via query params
        if st.query_params.get("event") == "main_owner_promo_click" and st.query_params.get("card") == "compact":
            log_analytics("main_owner_promo_click", {"card_size": "compact", "variant": variant})

        st.markdown(f"""
        <a href="?page=Main+Owner&event=main_owner_promo_click&card=compact" class="promo-card compact-card" aria-label="{compact_headlines[variant]}" target="_self">
            <div class="card-icon">{MAIN_OWNER_ICON}</div>
            <div class="card-content">
                <div class="card-headline">{compact_headlines[variant]}</div>
                <div class="card-body">Unify your agent's mission today.</div>
            </div>
            <div class="card-cta">{compact_ctas[cta_variant]}</div>
        </a>
        """, unsafe_allow_html=True)
