...
if "promo_variant" not in st.session_state:
    st.session_state.promo_variant = "A" if datetime.now().microsecond % 2 == 0 else "B"
if "cta_variant" not in st.session_state:
    st.session_state.cta_variant = "1" if datetime.now().microsecond % 2 == 0 else "2"

is_dark = st.session_state.theme == "dark"
promo_v = st.session_state.promo_variant
...
def log_analytics(event_name, payload=None):
    """Mock analytics event logging with robust deduplication."""
    if "logged_events" not in st.session_state:
        st.session_state.logged_events = set()

    # Create a unique key for the event in this session/run context
    event_key = f"{event_name}_{json.dumps(payload, sort_keys=True)}"
    if event_key in st.session_state.logged_events:
        return None

    st.session_state.logged_events.add(event_key)
...
with st.sidebar:
    ...
    log_analytics("main_owner_promo_view", {
        ...
        "variant": promo_v,
        "headline": mobile_headlines[promo_v],
        "location": "sidebar"
    })

    st.markdown(f"""
    <div class="promo-card mobile-tile" onclick="window.location.href='?page=Main+Owner+Guide&from=sidebar'">
        <div class="card-icon">{MAIN_OWNER_ICON}</div>
        <div class="card-headline">{mobile_headlines[promo_v]}</div>
        <a href="?page=Main+Owner+Guide&from=sidebar" class="card-cta" aria-label="{mobile_headlines[promo_v]} - {mobile_ctas[cta_v]}">{mobile_ctas[cta_v]}</a>
    </div>
    """, unsafe_allow_html=True)
...
if page == "Dashboard":
    ...
    <div class="promo-card hero-card" onclick="window.location.href='?page=Main+Owner+Guide&from=hero'">
        ...
        <div>
            <a href="?page=Main+Owner+Guide&from=hero" class="card-cta" aria-label="{hero_headlines[promo_v]} - {hero_ctas[cta_v]}">{hero_ctas[cta_v]}</a>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_right:
        ...
        log_analytics("main_owner_promo_view", {
            "card_size": "compact",
            "variant": promo_v,
            "headline": compact_headlines[promo_v],
            "location": "dashboard_right"
        })
        ...
        <div class="promo-card compact-card" onclick="window.location.href='?page=Main+Owner+Guide&from=compact'">
            ...
            <div class="card-content">
                <div class="card-headline">{compact_headlines[promo_v]}</div>
                <div class="card-body">Establish ultimate accountability and unified governance for your mission.</div>
            </div>
            <a href="?page=Main+Owner+Guide&from=compact" class="card-cta" aria-label="{compact_headlines[promo_v]} - {compact_ctas[cta_v]}">{compact_ctas[cta_v]}</a>
        </div>
        """, unsafe_allow_html=True)
...
# ─── MAIN OWNER GUIDE PAGE ──────────────────────────────
    ...
    # Analytics for Click tracking via 'from' parameter
    promo_source = st.query_params.get("from")
    if promo_source:
        # We need to reconstruct which variants were shown.
        # Since they are in session state, we can use them.
        headlines = {
            "hero": {"A": "Master Your AI Fleet", "B": "Unified Control Starts Here"},
            "compact": {"A": "Secure Your Workspace", "B": "Enable Owner Oversight"},
            "sidebar": {"A": "Main Owner Setup", "B": "Control Center"}
        }
        ctas = {
            "hero": {"1": "Set Main Owner", "2": "Start Onboarding"},
            "compact": {"1": "Setup Now", "2": "View Guide"},
            "sidebar": {"1": "Start", "2": "Unlock"}
        }

        if promo_source in headlines:
            log_analytics("main_owner_promo_click", {
                "card_size": promo_source,
                "variant": promo_v,
                "headline": headlines[promo_source][promo_v],
                "cta": ctas[promo_source][cta_v],
                "location": f"dashboard_{'top' if promo_source=='hero' else 'right'}" if promo_source != 'sidebar' else 'sidebar'
            })
    ...
    with col_s1:
        ...
        """, unsafe_allow_html=True)
        if st.button("Start Setup", key="start_setup_btn"):
            log_analytics("main_owner_setup_start", {"source": "onboarding_page"})
            st.info("Setup process initiated in Governance tab.")
    ...
    with col_s3:
        ...
        """, unsafe_allow_html=True)
        if st.button("Complete Setup", key="complete_setup_btn"):
            log_analytics("main_owner_setup_complete", {"source": "onboarding_page"})
            st.success("Main Owner identity confirmed and locked!")
