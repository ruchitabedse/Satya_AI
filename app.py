...
# A/B Testing Centralized Logic
# Using a simple time-based toggle for demo purposes
def get_ab_variants():
    now = datetime.now()
    return {
        "headline": "A" if now.second % 2 == 0 else "B",
        "cta": "1" if now.second % 4 < 2 else "2"
    }
...
with st.sidebar:
    ...
    # Centralized A/B Testing
    variants = get_ab_variants()
    ...
    # Log view for Sidebar Mobile Tile
    log_analytics("main_owner_promo_view", {"card_size": "mobile_tile", "variant": headline_variant, "cta": mobile_ctas[cta_variant]})

    st.markdown(f"""
    <a href="?page=Main+Owner+Guide" target="_self" class="promo-card mobile-tile" style="margin: 0.5rem; text-decoration: none;">
...
if page == "Dashboard":
    ...
    log_analytics("main_owner_promo_view", {"card_size": "hero", "variant": headline_variant, "cta": hero_ctas[cta_variant]})

    st.markdown(f"""
    <a href="?page=Main+Owner+Guide" target="_self" class="promo-card hero-card" style="text-decoration: none;">
    ...
    with col_right:
        ...
        log_analytics("main_owner_promo_view", {"card_size": "compact", "variant": headline_variant, "cta": compact_ctas[cta_variant]})

        st.markdown(f"""
        <a href="?page=Main+Owner+Guide" target="_self" class="promo-card compact-card" style="text-decoration: none; margin-bottom: 1rem;">
            <div class="card-icon">{MAIN_OWNER_ICON}</div>
...
# ─── MAIN OWNER PAGE ─────────────────────────────────────
    ...
    with col_s3:
        ...
        if st.button("Finalize Setup", use_container_width=True):
            ...
            st.success("Main Owner finalized! Centralized oversight enabled.")
