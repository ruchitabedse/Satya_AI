...
with st.sidebar:
    ...
    if st.button(f"{theme_icon} {theme_label}", key="theme_toggle", use_container_width=True, help=f"Switch to {'light' if is_dark else 'dark'} theme"):
    ...
    <div class="promo-card promo-mobile" style="margin: 0.5rem; padding: 1rem;">
        <div class="promo-icon" style="font-size: 1.5rem; margin-bottom: 0.5rem;"><span role="img" aria-label="Crown">👑</span></div>
...
if page == "Dashboard":
    ...
    with c1:
        ...
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="Chart">📊</span></div>
    ...
    with c2:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="Pin">📌</span></div>
    ...
    with c3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="Lightning">⚡</span></div>
    ...
    with c4:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="Check">✅</span></div>
    ...
    with col_left:
        ...
        if sorted_tasks:
            ...
            <div class="empty-state">
                <div class="empty-state-icon"><span role="img" aria-label="Clipboard">📋</span></div>
...
# ─── TASK BOARD PAGE ────────────────────────────────────
    ...
    headers = {
        "queued": ("header-todo", '<span role="img" aria-label="Pin">📌</span> To Do', col1),
        "in_progress": ("header-progress", '<span role="img" aria-label="Lightning">⚡</span> In Progress', col2),
        "done": ("header-done", '<span role="img" aria-label="Check">✅</span> Done', col3)
    }
        ...
        with col:
            ...
            if not tasks_by_status[status]:
                ...
                <div class="empty-state" style="padding: 2rem 1rem;">
                    <div style="font-size: 2rem; opacity: 0.3;"><span role="img" aria-label="Folder">📁</span></div>
            ...
            for task in tasks_by_status[status]:
                ...
                <div class="task-card {get_priority_class(priority)}">
                    ...
                    <div class="task-meta">
                        ...
                        <span role="img" aria-label="User">👤</span> {html.escape(task.get('assignee', 'Unassigned'))} &middot;
                        <span role="img" aria-label="Calendar">📅</span> {format_date(task.get('created_at', ''))}
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
# ─── TRUTH SOURCE PAGE ─────────────────────────────────
    ...
    if files:
        ...
        for idx, fname in enumerate(files):
            ...
            with col_f:
                ...
                <div class="truth-card">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <div style="font-weight: 600; color: var(--text-primary);"><span role="img" aria-label="Document">📄</span> {html.escape(fname)}</div>
        ...
        st.markdown("""
        <div class="empty-state">
            <div class="empty-state-icon"><span role="img" aria-label="Books">📚</span></div>
...
# ─── AGENT LOGS PAGE ───────────────────────────────────
    ...
    if not os.path.exists(storage.AGENTS_DIR):
        st.markdown("""
        <div class="empty-state">
            <div class="empty-state-icon"><span role="img" aria-label="Detective">🕵️</span></div>
