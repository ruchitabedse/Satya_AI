...
with st.sidebar:
    ...
    target_page = query_params.get("page", st.session_state.get("current_page", "Dashboard")).replace("+", " ")
    ...
    # Log view event for sidebar mobile tile
    log_analytics("main_owner_promo_view", {"card_size": "mobile", "variant": headline_variant, "location": "sidebar"})

    st.markdown(f"""
    <a href="?page=Main+Owner+Guide&clicked_promo=mobile&v={headline_variant}" class="promo-card mobile-tile" style="margin: 0.5rem; text-decoration: none;" aria-label="{mobile_headlines[headline_variant]} - {mobile_ctas[cta_variant]}">
        <div class="card-icon" role="img" aria-label="Main Owner Icon">{MAIN_OWNER_ICON}</div>
...
if page == "Dashboard":
    ...
    if "clicked_promo" in query_params:
        ...
        variant = query_params.get("v", "Unknown")
        log_analytics("main_owner_promo_click", {"card_size": promo_type, "variant": variant})
        # Clear params and rerun to avoid double logging
        st.session_state.current_page = "Main Owner Guide"
    ...
    # Main Owner Promo Hero Card (A/B Test Ready)
    headline_variant = st.session_state.headline_variant
    cta_variant = st.session_state.cta_variant
    ...
    log_analytics("main_owner_promo_view", {"card_size": "hero", "variant": headline_variant, "location": "dashboard_top"})

    st.markdown(f"""
    <a href="?page=Main+Owner+Guide&clicked_promo=hero&v={headline_variant}" class="promo-card hero-card" aria-label="{hero_headlines[headline_variant]} - {hero_ctas[cta_variant]}">
    ...
    with col_right:
        ...
        log_analytics("main_owner_promo_view", {"card_size": "compact", "variant": headline_variant, "location": "dashboard_sidebar"})

        st.markdown(f"""
        <a href="?page=Main+Owner+Guide&clicked_promo=compact&v={headline_variant}" class="promo-card compact-card" aria-label="{compact_headlines[headline_variant]} - {compact_ctas[cta_variant]}">
            <div class="card-icon" role="img" aria-label="Main Owner Icon">{MAIN_OWNER_ICON}</div>
...
# ─── MAIN OWNER PAGE ─────────────────────────────────────
    ...
    log_analytics("main_owner_setup_start", {"source": "navigation"})
    ...
    with col_s3:
        ...
        if st.button("Finalize Setup", use_container_width=True):
            ...
            st.success("Main Owner setup complete! (Simulated)")
