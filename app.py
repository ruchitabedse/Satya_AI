...
def parse_iso(iso_str):
    ...
    if not iso_str:
        return None
    try:
        # Robustly handle 'Z' and existing numeric offsets
        clean_iso = iso_str
        if clean_iso.endswith('Z'):
            clean_iso = clean_iso[:-1]
            # Only add +00:00 if no other timezone indicator is present
...
def format_time_ago(iso_str):
    """Formats an ISO string to a relative time ago string."""
    dt = parse_iso(iso_str)
    if not dt:
