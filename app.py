...
# Stabilize A/B variants for the session
if "ab_variants" not in st.session_state:
    now = datetime.now()
    st.session_state.ab_variants = {
        "headline": "A" if now.microsecond % 2 == 0 else "B",
        "cta": "1" if now.second % 2 == 0 else "2"
    }
...
def get_ab_variants():
    """Returns stable A/B test variants from session state."""
    return st.session_state.ab_variants["headline"], st.session_state.ab_variants["cta"]
...
# --- Global Navigation & Click Tracking Logic ---
...
if "clicked_promo" in query_params:
    p_size = query_params.get("clicked_promo")
    p_variant = query_params.get("v", "A")
    p_cta_variant = query_params.get("cta", "1")

    # Map variant to headline/cta for payload
    p_headlines = {
        "hero": {"A": "Master Your AI Fleet", "B": "Unified Control Starts Here"},
        "compact": {"A": "Secure Your Workspace", "B": "Enable Owner Oversight"},
        "mobile": {"A": "Main Owner Setup", "B": "Control Center"}
    }
    p_ctas = {
        "hero": {"1": "Set Main Owner", "2": "Start Onboarding"},
        "compact": {"1": "Setup Now", "2": "View Guide"},
        "mobile": {"1": "Start", "2": "Unlock"}
    }

    log_analytics("main_owner_promo_click", {
        "card_size": p_size,
        "variant": p_variant,
        "headline": p_headlines.get(p_size, {}).get(p_variant, "Unknown"),
        "cta": p_ctas.get(p_size, {}).get(p_cta_variant, "Unknown"),
        "location": "internal_link"
    })
    log_analytics("main_owner_setup_start", {"source": "main_owner_guide"})

    # Clear and reset to target page to avoid re-logging on refresh
    # Use the target_page captured earlier or fallback
    new_page = target_page if "target_page" in locals() else "Dashboard"
...
with st.sidebar:
    ...
    # Mobile Tile Promo
    headline_v, cta_v = get_ab_variants()
    mobile_headlines = {"A": "Main Owner Setup", "B": "Control Center"}
    mobile_ctas = {"1": "Start", "2": "Unlock"}

    log_analytics("main_owner_promo_view", {"card_size": "mobile", "variant": headline_v, "headline": mobile_headlines[headline_v], "cta": mobile_ctas[cta_v], "location": "sidebar"})
    ...
    st.markdown(f"""
    <a href="?page=Main+Owner+Guide&clicked_promo=mobile&v={headline_v}&cta={cta_v}" target="_self" class="promo-card mobile-tile" style="margin: 0.5rem;" aria-label="Main Owner Setup Promotion">
        <div class="card-icon" aria-hidden="true">{main_owner_icon}</div>
        <div class="card-headline">{mobile_headlines[headline_v]}</div>
...
if page == "Dashboard":
    ...
    log_analytics("main_owner_promo_view", {"card_size": "hero", "variant": headline_v, "headline": hero_headlines[headline_v], "cta": hero_ctas[cta_v], "location": "dashboard_top"})

    st.markdown(f"""
    <a href="?page=Main+Owner+Guide&clicked_promo=hero&v={headline_v}&cta={cta_v}" target="_self" class="promo-card hero-card" aria-label="Main Owner Onboarding Promotion">
        <div class="card-headline">{hero_headlines[headline_v]}</div>
    ...
    with col_right:
        ...
        log_analytics("main_owner_promo_view", {"card_size": "compact", "variant": headline_v, "headline": compact_headlines[headline_v], "cta": compact_ctas[cta_v], "location": "dashboard_sidebar"})

        st.markdown(f"""
        <a href="?page=Main+Owner+Guide&clicked_promo=compact&v={headline_v}&cta={cta_v}" target="_self" class="promo-card compact-card" aria-label="Main Owner Feature Promotion">
            <div class="card-icon" aria-hidden="true">{main_owner_icon}</div>
            <div class="card-content">
                <div class="card-headline">{compact_headlines[headline_v]}</div>
