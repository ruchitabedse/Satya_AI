...
def parse_iso(iso_str):
    ...
    if not iso_str:
        return None
    try:
        # Handle cases like '2023-10-27T10:00:00Z'
        clean_iso = iso_str
...
with st.sidebar:
    ...
    # Determine default_index for navigation
    ...
    <div style="padding: 0.5rem;">
        ...
        <div class="progress-bar-container" role="progressbar" aria-valuenow="{completion}" aria-valuemin="0" aria-valuemax="100">
    ...
    if "promo_variants" not in st.session_state:
        st.session_state.promo_variants = {
            "headline": "A" if datetime.now().second % 2 == 0 else "B",
            "cta": "1" if datetime.now().second % 4 < 2 else "2"
        }

    v = st.session_state.promo_variants
    ...
    st.markdown(f"""
    <div class="promo-card mobile-tile" title="Click to start onboarding">
        <div class="card-icon">{MAIN_OWNER_ICON}</div>
        <div class="card-headline">{mobile_headlines[v['headline']]}</div>
        <a href="?page=Main+Owner+Guide&clicked_promo=mobile&v={v['headline']}{v['cta']}" target="_self" class="card-cta" aria-label="Start Main Owner Onboarding">{mobile_ctas[v['cta']]}</a>
    </div>
    """, unsafe_allow_html=True)
...
# ─── DASHBOARD PAGE ─────────────────────────────────────
if page == "Dashboard":
    # Click tracking for promo cards
    if "clicked_promo" in query_params:
        promo_type = query_params["clicked_promo"]
        variant = query_params.get("v", "A1")
        log_analytics("main_owner_promo_click", {"card_size": promo_type, "variant": variant})
    ...
    log_analytics("main_owner_promo_view", {"card_size": "hero", "variant": v['headline']+v['cta']})

    st.markdown(f"""
    <div class="promo-card hero-card" title="Open Main Owner Onboarding Guide">
        <div class="card-headline">{hero_headlines[v['headline']]}</div>
        ...
        <div>
            <a href="?page=Main+Owner+Guide&clicked_promo=hero&v={v['headline']}{v['cta']}" target="_self" class="card-cta" aria-label="{hero_headlines[v['headline']]} - {hero_ctas[v['cta']]}">{hero_ctas[v['cta']]}</a>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c2:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="Pushpin">📌</span></div>
    ...
    with c3:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="High Voltage">⚡</span></div>
    ...
    with c4:
        st.markdown(f"""
        <div class="metric-card">
            <div class="metric-icon"><span role="img" aria-label="Check Mark Button">✅</span></div>
    ...
    with col_right:
        ...
        <div class="metric-card" style="margin-bottom: 1rem;">
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.3rem 0;">
                <span style="color: var(--text-secondary);"><span role="img" aria-label="Books">📚</span> Knowledge Sources</span>
                <span style="font-weight: 700; font-size: 1.2rem; color: var(--primary-light);">{len(truth_files)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.3rem 0;">
                <span style="color: var(--text-secondary);"><span role="img" aria-label="Detective">🕵️</span> Agent Logs</span>
                <span style="font-weight: 700; font-size: 1.2rem; color: var(--primary-light);">{len(log_files)}</span>
            </div>
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.3rem 0;">
                <span style="color: var(--text-secondary);"><span role="img" aria-label="Rocket">🚀</span> Completion Rate</span>
        ...
        log_analytics("main_owner_promo_view", {"card_size": "compact", "variant": v['headline']+v['cta']})

        st.markdown(f"""
        <div class="promo-card compact-card" title="Open Main Owner Onboarding Guide">
            <div class="card-icon">{MAIN_OWNER_ICON}</div>
            <div class="card-content">
                <div class="card-headline">{compact_headlines[v['headline']]}</div>
                <div class="card-body">Unify your agent's mission today.</div>
            </div>
            <a href="?page=Main+Owner+Guide&clicked_promo=compact&v={v['headline']}{v['cta']}" target="_self" class="card-cta" aria-label="{compact_headlines[v['headline']]} - {compact_ctas[v['cta']]}">{compact_ctas[v['cta']]}</a>
        </div>
        """, unsafe_allow_html=True)
...
# ─── MAIN OWNER PAGE ─────────────────────────────────────
    ...
    with col_s1:
        ...
        """, unsafe_allow_html=True)
        if st.button("Start Setup", key="setup_start", use_container_width=True):
            log_analytics("main_owner_setup_start", {"source": "documentation_page"})
            st.info("Navigate to Governance tab to begin.")
    ...
    with col_s3:
        ...
        """, unsafe_allow_html=True)
        if st.button("Complete Setup", key="setup_complete", use_container_width=True):
            log_analytics("main_owner_setup_complete", {"source": "documentation_page"})
            st.success("Setup confirmed! (Analytics Event Logged)")
    ...
    with col_spec:
        ...
        for item in checklist:
            st.markdown(f'<div style="font-size: 0.9rem; margin-bottom: 0.5rem;"><span role="img" aria-label="Checked">✅</span> {item}</div>', unsafe_allow_html=True)
